...
| `BoldSpinner` / `CoreflowBusy` | `DilettaLoadingSpinner` | `SpinnerSize` 3 nos dois |
...
| `CoreflowOperatingStrip` (2 usos) · `CoreflowOperatingSlot` (2) | `DilettaStatusBanner` | `CoreflowOperatingContext` tem **zero uso** — não portar |
...
próximo é o `CoreflowAbas`, que nasceu aqui — mesma API (`List<String>` + índice + `onChanged`) e outro
idioma visual (sublinhado contra pílula preenchida). Então o caminho provável é **variante do
`CoreflowAbas`**, não componente novo — e é decisão pra o próximo ciclo, com os 3 usos medidos.
...
| `CoreflowBackground` (+ `SecondaryBackground`, `HomeBackground`, `BackdropScope`, enum `CoreflowBackdrop`) | 291 | **pai** — backdrop com scrim de legibilidade é estrutura; os VALORES (a imagem, o wine-ink) ficam no filho |
...
|---|---|---|
| `CoreflowBackground` (+ scope, + enum de 7 fundos) | **114** | **nasceu** |
| `BoldSeloQuantico` (era `BoldQuantumSeal`) | 9 | **nasceu** — e tinha 3 defeitos |
| `CoreflowDinheiro` (era `BoldMoneyInputFormatter`) | 8 | **nasceu** — e o teto deslizava |
| `CoreflowSaldo` (era `BoldBalance`) | 3 | **nasceu** |
| `CoreflowCopiar` (era `BoldCopyButton`) · `CoreflowAbas` (era `BoldTabs`) | 3 cada | **nasceram** |
| `CoreflowCabecalhoDaHome` | 3 | **nasceu** — e é CASCA, não acessório |
| `CoreflowVisorDeCodigo` | 1 (e é o diferencial) | **nasceu** — sem plugin de câmera |
| `CoreflowResumoDaTransacao` (era o topo de `BoldTransactionSummary`) | **3** | **nasceu** — e só o CONTEÚDO |
| `CoreflowEscadaDeAlcadas` (era `BoldRuleLadder`) | 2 | **nasceu** — e o texto estava abaixo de AA |
| `CoreflowProgressoDeAprovacao` · `CoreflowPrazoDaPendencia` | 1 cada | **nasceram** — o par da pendência |
| `CoreflowSegmentos` (era `BoldSegmentedControl`) | 3 | **nasceu** — e NÃO é variante das abas |
| `CoreflowPontosDePagina` (era `BoldPageDots`) | 1 | **nasceu** — cor de papel, e o ativo alonga |
| `BoldPixMark` | 1 | **não nasce**: é `DilettaIcon(pixSolid)`. Rename, e a regra do BCB é doc |
| `BoldSecondaryBackground` | 1 | **não nasce**: é `CoreflowBackground(estilo: solido)`, que já existe |
...
O `BoldSecondaryBackground` é `CoreflowBackground(style: solid)` com outro nome — e o fundo que nasceu aqui
...
`CoreflowResumoDaTransacao`. A medição que justifica: **três telas de comprovante** (Pix, boleto, TED)
...
app. Agora a arte entra pelo `CoreflowBackdropScope` (que o app já declarava pro estilo), e sem arte o
...
**3 · A tipografia estava presa num estático.** `CoreflowType.fontFamily` era lido DENTRO do
...
| peça | o que é |
|---|---|
| `CoreflowVisorDeCodigo` | o overlay: cantos em colchete, varredura com rastro, segundo quadro em saltos de câmera, rótulo com linha de chamada, alvos fantasma. `CustomPainter` puro, zero dependência |
| `CoreflowFormatosDeCodigo` | a lista de formatos, com o motivo escrito. É a peça de conhecimento mais fácil de perder, e já custou bug de QA: o default da plataforma **não habilita 1D**, e o boleto brasileiro é ITF de 44 dígitos |
...
| `CoreflowBackground.veuDaStatusBar()` | **0** | **fica, e o consumidor é o APP** |
...
**Um saiu — e VOLTOU na `v0.6.0`, porque a medição estava errada.** `CoreflowDinheiro.emReais(String) →
...
*"o pacote externo `coreflow_design_system` não é mais consumido"*. O app voltou pro DS interno
...
| desses, que existem no pacote **com o mesmo nome** | **3** (`CoreflowBackdrop`, `CoreflowBackdropScope`, `CoreflowBackground`) |
| superfície do pacote | 23 `Bold*` + 193 `Diletta*` |
| chamadas nos seis símbolos mais pesados | `BoldSpace` 1269 · `AppColors` 611 · `CoreflowType` 505 · `BoldColors` 298 · `AppTextStyles` 278 · `AppSpacing` 248 |
...
(`all24`) tem o nome errado.** Quem traduzir por busca-e-substitui de `CoreflowRadius.card` →
...
- `CoreflowType.fontFamily = 'Poppins'` (`bold_typography.dart:18`);
...
Adotar conserta isso, e **muda a cara de todo texto do app de uma vez** — 505 chamadas de `CoreflowType`. É
