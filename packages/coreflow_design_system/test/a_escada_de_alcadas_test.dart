...
void main() {
  ...
  const duasFaixas = [
    CoreflowDegrauDeAlcada(ate: r'R$ 5.000,00', aprovacoes: 0),
    CoreflowDegrauDeAlcada(aprovacoes: 2, exigeMaster: true),
  ];

  testWidgets('lê a regra em palavras: até, acima de, e a exigência', (t) async {
    await t.pumpWidget(naTela(const CoreflowEscadaDeAlcadas(degraus: duasFaixas)));
  ...
  testWidgets('a FAIXA sai de dois degraus, não de um — "De X a Y"', (t) async {
    ...
    await t.pumpWidget(naTela(const CoreflowEscadaDeAlcadas(degraus: [
      CoreflowDegrauDeAlcada(ate: r'R$ 1.000,00', aprovacoes: 0),
      CoreflowDegrauDeAlcada(ate: r'R$ 10.000,00', aprovacoes: 1),
      CoreflowDegrauDeAlcada(aprovacoes: 3),
    ])));
    await t.pump(const Duration(milliseconds: 50));
  ...
  testWidgets('escada de um degrau terminal diz "Qualquer valor"', (t) async {
    await t.pumpWidget(naTela(const CoreflowEscadaDeAlcadas(
        degraus: [CoreflowDegrauDeAlcada(aprovacoes: 1)])));
    await t.pump(const Duration(milliseconds: 50));
  ...
  testWidgets('linha cheia não estoura — e o teste mede o PIOR caso, de propósito', (t) async {
    ...
    await t.pumpWidget(naTela(
      const CoreflowEscadaDeAlcadas(degraus: [
        CoreflowDegrauDeAlcada(ate: r'R$ 1.234.567,89', aprovacoes: 0),
        CoreflowDegrauDeAlcada(ate: r'R$ 9.876.543,21', aprovacoes: 4, exigeMaster: true),
        CoreflowDegrauDeAlcada(aprovacoes: 5),
      ]),
      largura: 280,
    ));
    await t.pump(const Duration(milliseconds: 50));
