...
void main() {
  ...
  group('copiar', () {
    ...
    Finder meuToque() => find
        .descendant(of: find.byType(CoreflowCopiar), matching: find.byType(DilettaTappable))
        .first;
    ...
    testWidgets('copia pro clipboard e mostra o aviso, que SOME depois', (t) async {
      var copiou = 0;
      await t.pumpWidget(montar(CoreflowCopiar(
        texto: 'chave-pix-123',
    ...
    testWidgets('o toque VIBRA, e é a única chamada tátil do produto', (t) async {
      ...
      await t.pumpWidget(montar(const CoreflowCopiar(
        texto: 'chave-pix-123',
        rotuloDeAcessibilidade: 'Copiar chave',
      )));
      await t.tap(meuToque());
  ...
  group('abas', () {
    testWidgets('marca a ativa por COR e por espessura, nos dois modos', (t) async {
      ...
      for (final escuro in [false, true]) {
        await t.pumpWidget(montar(
          CoreflowAbas(abas: const ['Tudo', 'Entradas', 'Saídas'],
        ...
        final tracos = t
            .widgetList<Container>(find.descendant(
                of: find.byType(CoreflowAbas), matching: find.byType(Container)))
    ...
    testWidgets('o toque pega a ABA inteira, não só o texto', (t) async {
      ...
      var trocou = -1;
      await t.pumpWidget(montar(CoreflowAbas(
        abas: const ['A', 'B'],
        indiceSelecionado: 0,
        aoTrocar: (i) => trocou = i,
      )));
