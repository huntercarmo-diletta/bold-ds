...
void main() {
  ...
  test('o vidro do app lê a receita da paleta, e ela não é nula', () {
    ...
    expect(CoreflowVidro.blur(p), p.blurDeVidro);
    expect(CoreflowVidro.tinte(p, escuro: false), p.tinteDeVidroClaro);
    expect(CoreflowVidro.tinte(p, escuro: true), p.tinteDeVidroEscuro);
    expect(CoreflowVidro.traco(p, escuro: false), p.tracoDeVidroClaro);
    expect(CoreflowVidro.traco(p, escuro: true), p.tracoDeVidroEscuro);
  });

  test('e os cinco valores são os que o app desenhava — medidos em 19/08', () {
    expect(CoreflowVidro.blur(p), 15);
    expect(CoreflowVidro.tinte(p, escuro: true).toARGB32(), 0x8016060A); // vinho-tinta a 50%
    expect(CoreflowVidro.tinte(p, escuro: false).toARGB32(), 0x80FFFFFF); // branco a 50%
    expect(CoreflowVidro.traco(p, escuro: true).toARGB32(), 0x4DFF9898); // rosa claro a 30%
    expect(CoreflowVidro.traco(p, escuro: false), BoldColors.primary08);
    expect(CoreflowVidro.espessuraDoTraco, 1);
    expect(CoreflowVidro.sombra, isEmpty);
  });
  ...
  test('o tinte escuro é o vinho-tinta a 50%, e isso é conta e não coincidência', () {
    ...
    final tinte = CoreflowVidro.tinte(p, escuro: true);
  ...
  test('o vidro de ENTRADA é outro material, e não vira variante do primeiro', () {
    ...
    expect(CoreflowVidroDeEntrada.blur, isNot(CoreflowVidro.blur));
    expect(CoreflowVidroDeEntrada.base(p, escuro: true), BoldVinho.lavagem);
    expect(CoreflowVidroDeEntrada.base(p, escuro: false), BoldColors.primary09);
    expect(CoreflowVidroDeEntrada.opacidade(escuro: true), 0.60);
    expect(CoreflowVidroDeEntrada.opacidade(escuro: false), 0.70);
    expect(CoreflowVidroDeEntrada.sombra, isEmpty);

    // O gradiente sobe: opaco embaixo, transparente no topo. É o que deixa a cidade aparecer.
    final g = CoreflowVidroDeEntrada.gradiente(p, escuro: true);
