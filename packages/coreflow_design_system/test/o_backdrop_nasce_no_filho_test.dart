...
void main() {
    ...
    for (final escuro in [false, true]) {
      for (final fundo in CoreflowBackdrop.values) {
        await t.pumpWidget(montar(
          CoreflowBackground(estilo: fundo, child: const Text('conteúdo')),
          escuro: escuro,
        ));
  ...
  testWidgets('sem arte declarada, o fundo de imagem DEGRADA — não quebra', (t) async {
    ...
    await t.pumpWidget(montar(
      const CoreflowBackground(estilo: CoreflowBackdrop.imagem, child: Text('conteúdo')),
    ));
    await t.pump(const Duration(milliseconds: 50));
    ...
    await t.pumpWidget(MaterialApp(
      home: DilettaThemeScope(
        ...
        child: CoreflowBackdropScope(
          ...
          child: const Scaffold(body: CoreflowBackground(child: Text('conteúdo'))),
        ),
      ),
    ));
    await t.pump(const Duration(milliseconds: 50));
  ...
  testWidgets('a cor do fundo vem da PALETA e do papel, nunca de literal', (t) async {
    ...
    await t.pumpWidget(montar(
      const CoreflowBackground(estilo: CoreflowBackdrop.solido, child: SizedBox()),
      escuro: true,
    ));
    await t.pump(const Duration(milliseconds: 50));
  ...
  testWidgets('ZERO valor de cor solto: os fundos frios são VINHO, não violeta', (t) async {
    ...
    for (final fundo in [CoreflowBackdrop.vidroFrio, CoreflowBackdrop.aurora]) {
      await t.pumpWidget(montar(
        CoreflowBackground(estilo: fundo, child: const SizedBox()),
        escuro: true,
      ));
      await t.pump(const Duration(milliseconds: 50));
