...
///
/// Na primeira medição a resposta era **não**. O `CoreflowScheme` cravava `BoldPalette.bold` por dentro
...
void main() {
  ...
  Map<String, Color> papeis(CoreflowScheme s) => {
  ...
  test('o NETO troca a paleta e recebe o esquema DELE — a dívida é ZERO, e 35 dos 42 viajam', () {
    ...
    for (final brilho in [Brightness.light, Brightness.dark]) {
      final meu = papeis(CoreflowScheme.de(BoldPalette.bold, brilho: brilho));
      final dele = papeis(CoreflowScheme.de(neto, brilho: brilho));
