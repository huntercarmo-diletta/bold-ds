...
void main() {
  ...
  testWidgets('o ESTADO decide o ícone e o tom do spot — o par que era ternário na tela', (t) async {
    ...
    await t.pump(const Duration(milliseconds: 50));
    expect(iconeDe(CoreflowEstadoDaTransacao.concluida), DilettaIcons.circleCheckLight);
    ...
    await t.pumpWidget(naTela(const CoreflowResumoDaTransacao(
      ...
      valor: r'R$ 90,00',
      quando: 'Para 12/08',
      estado: CoreflowEstadoDaTransacao.agendada,
    )));
    await t.pump(const Duration(milliseconds: 50));
    expect(iconeDe(CoreflowEstadoDaTransacao.agendada), DilettaIcons.calendarLight);
    expect(t.widget<DilettaSpotIcon>(find.byType(DilettaSpotIcon)).state,
