...
void main() {
  ...
  testWidgets('entra na barra do PAI e aparece, nos dois modos', (t) async {
    for (final escuro in [false, true]) {
      await t.pumpWidget(naBarra(
        const CoreflowCabecalhoDaHome(nome: 'Ana', conta: 'Conta PF'),
        escuro: escuro,
      ));
  ...
  testWidgets('é CASCA e não acessório — as duas linhas não caberiam na barra', (t) async {
    ...
    await t.pumpWidget(naBarra(const CoreflowCabecalhoDaHome(nome: 'Ana', conta: 'Conta PF')));
    await t.pump(const Duration(milliseconds: 50));
    ...
    expect(t.getSize(find.byType(CoreflowCabecalhoDaHome)).height, 106,
  ...
  testWidgets('sem troca de conta NÃO tem chevron', (t) async {
    ...
    var trocou = 0;
    await t.pumpWidget(naBarra(CoreflowCabecalhoDaHome(
      nome: 'Ana',
      conta: 'Conta PF',
  ...
  testWidgets('nome vazio cai no "?" em vez de quebrar', (t) async {
    ...
    await t.pumpWidget(naBarra(const CoreflowCabecalhoDaHome(nome: '')));
    await t.pump(const Duration(milliseconds: 50));
    expect(t.takeException(), isNull);
  ...
  testWidgets('conta carregando mostra skeleton, não o rótulo vazio', (t) async {
    await t.pumpWidget(naBarra(const CoreflowCabecalhoDaHome(
      nome: 'Ana',
      conta: 'Conta PF',
  ...
  testWidgets('o mini-avatar NÃO é branco cravado — ele segue a superfície do tema', (t) async {
    ...
    await t.pumpWidget(naBarra(const CoreflowCabecalhoDaHome(nome: 'Ana'), escuro: true));
    await t.pump(const Duration(milliseconds: 50));
