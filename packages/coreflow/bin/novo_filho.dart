...
String cssDoProduto() => [
      ...
      coreflowGradientesCss(${op.id}.gradientes),
...
Map<String, Color> get _paradasDoLockup => {
  for (var i = 0; i < ${op.id}.gradientes.paradasDoLockup.length; i++)
    'lockup\${(i + 1).toString().padLeft(2, '0')}': ${op.id}.gradientes.paradasDoLockup[i],
};
...
```
sh tool/espelha_o_web.sh <tag-do-monorepo> ${_arquivo(op.id).replaceAll('_', '-')} [--seco]
```

Ele lê a SUA versão do `package.json` daqui — não a da tag —, embute o avô em `avo/` com recibo, e
cria a tag `${_arquivo(op.id).replaceAll('_', '-')}-web-v<a sua versão>`. Cada filho versiona
