...
void main() {
  test('todo degrau carrega a família da marca', () {
    for (final e in BoldType.todos.entries) {
      expect(e.value.fontFamily, BoldType.fontFamily, reason: e.key);
  ...
  test('os sete que derivam DERIVAM — px, altura, peso e tracking', () {
    ...
    igual(BoldType.headlineMd, DilettaType.headlineMd, 'headlineMd');
    igual(BoldType.headlineSm, DilettaType.headlineSm, 'headlineSm');
    igual(BoldType.titleMd, DilettaType.titleMd, 'titleMd');
    igual(BoldType.bodyLg, DilettaType.bodyLg, 'bodyLg');
    igual(BoldType.labelMd, DilettaType.labelMd, 'labelMd');
    igual(BoldType.bodySm, DilettaType.bodySm, 'bodySm');
    igual(BoldType.labelSm, DilettaType.labelSm, 'labelSm');

    // O saldo é o único que acrescenta: sem tabular o dígito muda de largura e o valor pula.
    expect(BoldType.headlineMd.fontFeatures, isNotEmpty);
  });
  ...
  test('a escada não tem dois degraus no mesmo lugar', () {
    ...
    porTamanho.forEach((px, nomes) {
      ...
      final assinaturas = nomes
          .map((n) => '${BoldType.todos[n]!.fontWeight}/${BoldType.todos[n]!.letterSpacing}')
          .toSet();
  ...
  test('o valor herói é UM, e ele é 32', () {
    expect(BoldType.valorHeroi.fontSize, 32);
    expect(BoldType.valorHeroi.fontFeatures, isNotEmpty);
    // Ele não é o `display`: aquele é o saldo de tela cheia, este é o valor da transação.
    expect(BoldType.display.fontSize, isNot(BoldType.valorHeroi.fontSize));
  });
}
