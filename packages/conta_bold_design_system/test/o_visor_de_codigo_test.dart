...
void main() {
  ...
  BoldAlvo alvo(BoldAlvoEstado estado, {String? rotulo, bool centralizado = false}) =>
      BoldAlvo(
  ...
  testWidgets('os TRÊS estados desenham, nos dois modos e em várias fases', (t) async {
    for (final escuro in [false, true]) {
      for (final estado in BoldAlvoEstado.values) {
  ...
  testWidgets('sem tamanho de imagem, cai no retículo CENTRAL em vez de desenhar errado', (t) async {
    ...
    await t.pumpWidget(montar(BoldVisorDeCodigo(
      alvos: [alvo(BoldAlvoEstado.analisando, centralizado: true)],
      fase: 0.3,
    )));
    await t.pump(const Duration(milliseconds: 30));
    expect(t.takeException(), isNull);
  ...
  testWidgets('rótulo longo não estoura a tela — ele quebra e clampa', (t) async {
    ...
    await t.pumpWidget(montar(BoldVisorDeCodigo(
      alvos: [
        alvo(BoldAlvoEstado.irrelevante,
  ...
  test('os FORMATOS que este produto lê estão declarados, com o boleto explícito', () {
    ...
    expect(BoldFormatosDeCodigo.todos, contains('qrCode'));
    expect(BoldFormatosDeCodigo.boleto, contains('itf2of5'),
        reason: 'sem ITF, boleto de 44 dígitos não é reconhecido — foi bug de QA');
    expect(BoldFormatosDeCodigo.todos.length, 7);
    // Sem duplicata: lista com nome repetido é lista que alguém editou duas vezes.
    expect(BoldFormatosDeCodigo.todos.toSet().length,
        BoldFormatosDeCodigo.todos.length);
  });
}
