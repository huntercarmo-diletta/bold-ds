...
void main() {
  ...
  const itens = [
    BoldItemDeNav(icone: DilettaIcons.houseLight, rotulo: 'Início'),
    BoldItemDeNav(icone: DilettaIcons.cameraLight, rotulo: 'Câmera'),
    BoldItemDeNav(icone: DilettaIcons.sparklesLightFull, rotulo: 'Letti'),
  ];

  testWidgets('ela NÃO traz o indicador de home por dentro', (t) async {
    await t.pumpWidget(naTela(
      const BoldNavFlutuante(itens: itens, ativo: 0),
    ));
  ...
  testWidgets('ela ABRAÇA os itens — hug, e não a largura da tela', (t) async {
    await t.pumpWidget(naTela(const Center(
      child: BoldNavFlutuante(itens: itens, ativo: 1),
    )));
    await t.pump(const Duration(milliseconds: 200));
  ...
  testWidgets('o item inativo guarda o espaço do spot — a fila não pula', (t) async {
    ...
    await t.pumpWidget(naTela(const BoldNavFlutuante(itens: itens, ativo: 0)));
    await t.pump(const Duration(milliseconds: 200));
    final comCameraInativa = alturaDoItem();

    await t.pumpWidget(naTela(const BoldNavFlutuante(itens: itens, ativo: 1)));
    await t.pump(const Duration(milliseconds: 200));
