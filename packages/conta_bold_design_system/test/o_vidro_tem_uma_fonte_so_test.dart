...
void main() {
  test('o vidro do app lê a receita da paleta, e ela não é nula', () {
    ...
    expect(BoldVidro.blur, p.blurDeVidro);
    expect(BoldVidro.tinte(escuro: false), p.tinteDeVidroClaro);
    expect(BoldVidro.tinte(escuro: true), p.tinteDeVidroEscuro);
    expect(BoldVidro.traco(escuro: false), p.tracoDeVidroClaro);
    expect(BoldVidro.traco(escuro: true), p.tracoDeVidroEscuro);
  });

  test('e os cinco valores são os que o app desenhava — medidos em 19/08', () {
    expect(BoldVidro.blur, 15);
    expect(BoldVidro.tinte(escuro: true).toARGB32(), 0x8016060A); // vinho-tinta a 50%
    expect(BoldVidro.tinte(escuro: false).toARGB32(), 0x80FFFFFF); // branco a 50%
    expect(BoldVidro.traco(escuro: true).toARGB32(), 0x4DFF9898); // rosa claro a 30%
    expect(BoldVidro.traco(escuro: false), BoldColors.primary08);
  ...
  test('o tinte escuro é o vinho-tinta a 50%, e isso é conta e não coincidência', () {
    ...
    final tinte = BoldVidro.tinte(escuro: true);
  ...
  test('o vidro de ENTRADA é outro material, e não vira variante do primeiro', () {
    ...
    expect(BoldVidroDeEntrada.base(escuro: true), BoldVinho.lavagem);
    expect(BoldVidroDeEntrada.base(escuro: false), BoldColors.primary09);
    ...
    final g = BoldVidroDeEntrada.gradiente(escuro: true);
