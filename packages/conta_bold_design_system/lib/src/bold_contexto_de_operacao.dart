...
/// **BoldOperatingContext** — declara, uma vez só no topo da árvore, que o
...
class BoldOperatingContext extends InheritedWidget {
  const BoldOperatingContext({
    super.key,
  ...
  static BoldOperatingContext? of(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<BoldOperatingContext>();

  @override
  bool updateShouldNotify(BoldOperatingContext old) =>
...
/// [BoldOperatingContext] acima na árvore.
class BoldOperatingStrip extends StatelessWidget {
  const BoldOperatingStrip({
    super.key,
    required this.accountName,
    this.role,
    this.onTap,
  ...
  static Widget? maybeOf(BuildContext context) {
    final ctx = BoldOperatingContext.of(context);
    if (ctx == null || ctx.accountName.isEmpty) return null;
    return BoldOperatingStrip(
          ...
          child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Flexible(
              child: Text(
                  ...
                  style: BoldType.labelSm.copyWith(color: c.primary)),
            ),
            if (onTap != null) ...[
              const SizedBox(width: DilettaSpacing.s2),
...
class BoldOperatingSlot extends StatelessWidget {
  const BoldOperatingSlot({super.key, this.safeTop = false});
  ...
  @override
  Widget build(BuildContext context) {
    final strip = BoldOperatingStrip.maybeOf(context);
