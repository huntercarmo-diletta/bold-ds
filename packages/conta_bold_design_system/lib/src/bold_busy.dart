...
/// Escopo de "ação em voo" — publicado por [BoldBusy], lido pelos inputs do DS.
///
...
class BoldBusyScope extends InheritedWidget {
  const BoldBusyScope({super.key, required this.busy, required super.child});

  final bool busy;

  /// `true` quando existe uma ação em voo acima deste ponto da árvore.
  static bool of(BuildContext context) =>
      context.dependOnInheritedWidgetOfExactType<BoldBusyScope>()?.busy ??
      false;

  @override
  bool updateShouldNotify(BoldBusyScope old) => busy != old.busy;
}

/// **BoldBusy** — trava o formulário enquanto a ação está em voo.
///
...
///
/// 1. **Desabilita os inputs do DS** pelo [BoldBusyScope]. `AbsorbPointer`
...
class BoldBusy extends StatefulWidget {
  const BoldBusy({
    super.key,
    required this.busy,
    required this.child,
  ...
  @override
  State<BoldBusy> createState() => _BoldBusyState();
}

class _BoldBusyState extends State<BoldBusy> {
  @override
  void didUpdateWidget(BoldBusy old) {
