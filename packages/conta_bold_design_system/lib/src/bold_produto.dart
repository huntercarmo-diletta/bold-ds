...
///
/// Esta classe é **a porta que não existia**. Até 20/08 o `BoldTheme` e o `BoldTemaMaterial`
/// cravavam `BoldPalette.bold` em quatro pontos, e o único lugar parametrizado da pilha era o
/// [BoldScheme.de] — que aceita paleta e **não tinha ninguém acima dele que aceitasse**. Medido: um
...
/// ```
///
/// **O par claro/escuro sai do mesmo objeto de propósito.** O `///` do [BoldTemaMaterial] já dizia
...
class BoldProduto {
  ...
  static final BoldProduto bold =
      BoldProduto(paleta: BoldPalette.bold, marca: marcaDoBold);
  ...
  late final BoldScheme esquemaClaro = BoldScheme.de(paleta, brilho: Brightness.light);

  /// O esquema de cor deste produto no escuro.
  late final BoldScheme esquemaEscuro = BoldScheme.de(paleta, brilho: Brightness.dark);
  ...
  late final ThemeData materialClaro = BoldTemaMaterial.de(esquemaClaro);

  /// O `ThemeData` do Material no escuro.
  late final ThemeData materialEscuro = BoldTemaMaterial.de(esquemaEscuro);
  ...
  late final BoldGradients gradientes = _gradientes ?? BoldGradients.bold;
  final BoldGradients? _gradientes;
}
