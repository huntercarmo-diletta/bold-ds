...
///
/// - **[BoldVisorDeCodigo]**, o overlay: cantos em colchete, varredura com rastro, o segundo quadro
///   em saltos de câmera de cinema, o rótulo com linha de chamada, e os alvos fantasma. 100%
///   `CustomPainter`, zero dependência;
/// - **[BoldFormatosDeCodigo]**, a lista de formatos que este produto precisa ler. É a peça de
...
abstract final class BoldFormatosDeCodigo {
...
class BoldVisorDeCodigo extends StatelessWidget {
  const BoldVisorDeCodigo({
    super.key,
    required this.alvos,
    required this.fase,
    this.descendo = true,
  ...
  final bool descendo;
  final List<BoldAlvoFantasma> fantasmas;
...
class _PintorDoVisor extends CustomPainter {
  ...
  Color _cor(BoldAlvoEstado e) => switch (e) {
        ...
        BoldAlvoEstado.irrelevante => _p.error05,
        BoldAlvoEstado.analisando => _p.warning04,
      };

  /// Mapeia a área da IMAGEM pro canvas assumindo `BoxFit.cover` — é como o preview preenche.
  Rect _paraCanvas(BoldAlvo a, Size size) {
    if (a.centralizado ||
  ...
  void _segundoQuadro(
      Canvas canvas, BoldAlvo alvo, Rect destino, Size size, Color cor) {
  ...
  void _desenhaFantasmas(Canvas canvas, Size size) {
    ...
    for (final f in fantasmas) {
      final t = agora.difference(f.inicio).inMilliseconds /
          BoldAlvoFantasma.duracao.inMilliseconds;
      if (t < 0 || t > 1) continue;
