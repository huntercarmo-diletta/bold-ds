...
| 3 | o gerador MORA no pai (`dart run coreflow:novo_filho`) e o filho gerado depende só de `coreflow`; exemplo regenerado e compilado; o Bold monta o tema pelo construtor de paleta INTEIRA (o adendo de 08/09 derrubou o `daMarca` como caminho dele) | `o_gerador_de_filho_tem_saida_conferida` (no pai, com a régua de valor: um hex só), `bold_e_filho_do_ds` |
