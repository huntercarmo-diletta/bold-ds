...
| [o respiro da casca desce dois pixels](avisos/2026-08-05-o-respiro-da-casca-desce-dois-pixels.md) | RELEASE | sim — a casca montada sai de **108 pra 106**, o `greaterThan(52)` virou número exato (controle: `Expected: <106> Actual: <108.0>` na v0.47.0), e a auditoria do ledger achou **duas linhas minhas dizendo *sem veredito* com veredito** — o débito de `pending` estava escondido ali · **2ª rodada (v0.25.1)**: a nota dele levou a minha quarta forma (asserção frouxa) pro documento de gate e achou 2 no lado dele; rodei aqui — 63 frouxas, 9 de geometria, **3 escondiam número e 1 saiu** · **3ª rodada (v0.25.2)**: o meu teto sentado no limite virou a SEGUNDA pergunta do crivo dele (*o valor medido está em cima do limite?*), que derrubou 3 dos 13 pisos que ele mantinha — aqui achou **7**, e **5 estavam fora da geometria** (contagens); carteira de sistema em pai apertado, que ele pediu pra medir: **zero telas** · **4ª rodada (v0.25.3)**: fui procurar a classe dele (contagem frouxa deixando doc derivar) e achei o estrago sem o teto — **7 números vivos errados em 2 arquivos**, o pior no cabeçalho de escopo do `ds_do_bold` (*"São 12 blocos"*, são 56). Sem gate de regex: o discriminador é tempo verbal, e história fica · **5ª rodada (v0.25.4)**: sem aviso — o pai da FERRAMENTA rodou a varredura no motor dele e achou 1 asserção medindo o objeto errado. A terceira pergunta (*o valor medido é o que o `reason:` fala?*) derrubou a minha **"a base é fixa"**, que media *"pinta depois do último bloco"* e passava nos dois mundos · **6ª rodada (v0.25.5)**: o gate de números dele não pegou os seis porque a lista de substantivos era FECHADA — e a minha varredura de ontem era fechada também, com as cinco palavras que eu já sabia erradas. A aberta dele achou **8 números vivos** aqui, 3 deles aproximações com `~`. E as 12 opcionais da paleta dele: **declaro as 12**, medido · **7ª rodada (v0.25.6)**: o **39** que eu escrevi ontem estava ERRADO — meu regex contava linha de declaração, o mesmo defeito do `grep -c` dele. São **53**, e o número passou a carregar o comando que responde a pergunta certa. Reconferi os outros números das rodadas: runtime passa por construção, os dois de regex remedidos pela via certa |
