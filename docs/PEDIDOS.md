...
| [o pacote web não sai do monorepo — o `path:` que o pub tem, o npm não tem](pedidos/2026-09-11-o-pacote-web-nao-sai-do-monorepo.md) | **sem veredito** — os 25 custom elements do `diletta_design_system_web` não podem ser instalados de fora do repo: `private: true`, subpasta de monorepo npm (e o npm não tem o `path:` do pub) e dois `exports` apontando pra fora da pasta, então o CSS não viaja no `files`. O custo já está pago: o IB mantém **195 tokens transcritos à mão e zero gates ligando-os à fonte**. Bloqueante: é a única coisa do lado web que não dá pra fazer daqui | — |
