...
| [o logo tem UMA arte e a página tem DUAS](pedidos/2026-09-14-o-logo-tem-uma-arte-e-a-pagina-tem-duas.md) | **sem veredito** — peço arte de logo por BRILHO (positivo pra página clara, negativo pra escura), no molde do `DilettaIllustration`, que já resolve este eixo em 27 pares. Medido: fora da faixa L 0,51–0,67 (**17 de 95 passos**) nenhuma cor chapada passa em 3:1 contra as duas páginas, o teto de uma arte só é **4,29:1 / 4,16:1**, e **4 de 6 cores de banco reprovam** numa das versões. Não peço que o `currentColor` saia: ele resolve o caso comum | — **escrito e pushado em 14/09; falta o SINAL.** **RETIFICADO em 15/09** (seção no fim do arquivo, commitada e esperando envio), com três coisas que não estavam nele: (1) o precedente do próprio pai — `DilettaSeloDeLoja` e `DilettaCarteiraDeSistema` já viajam em PAR claro/escuro no MESMO plugue de marca, com o `///` dizendo *«marca preta some no tema escuro»* como defeito dele na v0.28.0, enquanto `logo`/`logoFull` são caminho único (`diletta_brand_assets.dart:116-117`); (2) a decisão dela de 15/09 17h30, com manual de marca na mão — *«muda o "logo para fundo escuro" para o logo positivo/negativo, se é que essa versão existe no design system»*, e a resposta medida é **não existe pro logo, existe pro selo e pra carteira**; (3) a **D75 fundida aqui**, mantendo a numeração mais antiga, porque dois pedidos sobre o mesmo eixo contam como dois na régua de promoção dele |
...
| [a cópia campo a campo perde campo — e perdeu](pedidos/2026-09-17-a-copia-campo-a-campo-perde-campo-e-perdeu.md) | **sem veredito** — peço `DilettaBrand.copyWith`. O plugue tem 12 campos e a minha cópia manual levava 11: `nomeDaMarca` sumia do tema em TODO filho que não declara `corDoLogo` (nenhum declara), sem erro, sem teste vermelho e sem exceção — e o campo é o rótulo de leitor de tela da co-marca, que até a `v0.185.0` dele caía em `?? "CPF Seguro"`, o nome de outro produto. A recusa dele de 04/08 media a PALETA (67 campos, sem igualdade de valor); esta classe tem **12 campos e tem `==`/`hashCode`**, e a condição que ele escreveu (*"com o gate de que todo campo é carregado"*) vem CUMPRIDA junto: `a_marca_do_modo_nao_perde_campo_test` conta os campos lendo o arquivo dele e falha quando um não é copiado | — **escrito em 17/09; falta o SINAL.** Do meu lado já foi: `nomeDaMarca` entrou na cópia e o gate passa nos dois sentidos |
