...
| [o diálogo não tem onde pôr um campo](pedidos/2026-08-09-o-dialogo-nao-tem-onde-por-um-campo.md) | **aberto** — slot `content` no `DilettaDialog`. A divisão apareceu numa auditoria, não numa tela: dos **5 `AlertDialog`** do app, 3 eram título+mensagem+ações (couberam no diálogo) e **2 tinham CAMPO no meio**. Mandei os 3 pra folha e não me serve — **folha convive com a tela, diálogo interrompe**, e *"digite o código do convite"* é interrupção | — |
| [a barra de progresso não muda de tom](pedidos/2026-08-09-a-barra-de-progresso-nao-muda-de-tom.md) | **aberto** — `state` na `DilettaProgressBar`. É o OPOSTO do pedido do spot: a `.value` dele cobriu a barra de passo inteira sem eu pedir nada. O que sobrou é o medidor de LIMITE, onde a cor é a informação (primary → warning em 80% → danger no estouro). **As duas irmãs já mudam de tinta por estado** — `SpotIcon` e `StatusTag`; a barra é a única que não | — |
