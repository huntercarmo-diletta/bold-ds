...
| [o conjunto copia e não cola](pedidos/2026-08-08-o-conjunto-copia-e-nao-cola.md) | **aberto** — `clipboard-light`. Não é buraco de uma tela: **três telas do mesmo app tinham três respostas** (prancheta-com-checklist, `clone-light` que é o gesto CONTRÁRIO, e um `IconData` do Material). E o substituto não é neutro — ele já significa *documento conferido* em 7 sítios daqui, então colar e conferir passaram a ter o mesmo desenho | — |
| [a linguagem mostra valor e não recebe valor](pedidos/2026-08-08-a-linguagem-mostra-valor-e-nao-recebe-valor.md) | **aberto** — entrada de valor (`DilettaAmountInput`). O dinheiro tem dois gestos e a linguagem só tem um: **três** peças pra MOSTRAR valor, zero pra RECEBER. 6 sítios, e eles pediram os mesmos **dois portes** (hero e médio) que o `DilettaAmountDisplay` já tem do lado da leitura, sem combinar. Vai com um defeito MEU declarado: o `onChanged` daqui devolve `double` de um componente que contava em centavos | — |
| [o spot herói não existe, e cada tela inventou o dela](pedidos/2026-08-08-o-spot-heroi-nao-existe-e-cada-tela-inventou-o-dela.md) | **aberto** — `DilettaSpotState.brand`. A medição É a discordância: **seis telas, quatro diâmetros, três formas**, todas desenhando `gradient: BoldGradients.brand` à mão. O `SpotIcon` dele já cobre a geometria (`size` livre); o que não cobre é a TINTA — `state: primary` é um tinte, certo aos 34 numa linha de lista e desbotado aos 110 no centro da tela. **Não peço o número**: eu tenho quatro, que é a prova de que ninguém aqui decidiu | — |
