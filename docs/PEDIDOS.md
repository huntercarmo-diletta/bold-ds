...
| [a lista de candidatos anda numa direção só](pedidos/2026-08-10-a-lista-de-candidatos-anda-numa-direcao-so.md) | **aberto** — ordenar os candidatos por distância das tintas, em vez de percorrer a rampa na ordem dela. A derivação funcionou onde a DIREÇÃO da busca ajudava e piorou onde não: `normal` 2,93 → **1,32** e `error` **3,11 → 1,41** (passava antes). A curva não é monotônica — o rosa desta marca tem luminância média, e descer o trilho o aproxima da tinta antes de afastar. **Só o `neutral01` fecha, e ele está depois do fim da lista** | — |
